...
Investigating (Veteran Status) on Oahu at the census tract level.

```{r, message=FALSE, warning=FALSE, results='hide'}

oahu_veteran_status <- get_acs(
  geography = "tract",
  variables = c(veteran_status = "DP02_0070P"),
  state = "HI",
  county = "Honolulu",
  geometry = TRUE,
  output = "wide",
...
```{r, message=FALSE, warning=FALSE, results='hide'}
veteran_status_map <- ggplot(oahu_veteran_status) +
  geom_sf_interactive(
    aes(
      fill = veteran_statusE,
      tooltip = paste(NAME, ": ", veteran_statusE, "%"),
      data_id = NAME
    )
  ) +
  scale_fill_viridis_c(option = "magma") +
...
girafe(ggobj = veteran_status_map, width = 700, height = 400)
```
...
veteran_status_2012_2022 <- map_dfr(years, ~{
  get_acs(
    geography = "county",
    variables = c(percent_veterans = "DP02_0070P"),
    state = "HI",
    county = "Honolulu",
    year = .x,
    survey = "acs5"
...
```{r, warning=FALSE}
ggplot(veteran_status_2012_2022, aes(x = year, y = estimate, group = 1)) + 
  geom_line() + 
  geom_point()
```
...
Exploring the relationship between (Veteran Status) and (Poverty Levels) with a scatter plot

```{r, message=FALSE, warning=FALSE, results='hide'}
oahu_veteran_status_poverty <- get_acs(
  geography = "tract",
  variables = c(percent_veterans = "DP02_0070P",
                percent_poverty = "DP03_0119P"),
  state = "HI",
  county = "Honolulu",
  #geometry = TRUE,
  output = "wide",
...
```{r}
ggplot(oahu_veteran_status_poverty, aes(x = percent_veteransE, y = percent_povertyE)) +
  geom_point() +
  geom_smooth() +
  stat_cor() +
  theme_minimal()
...
We had no issues with the initial mapping of just the veteran status of people over the age of 18 which gave us the initial map above showing most counties on Oahu being between \~5 and 15% veterans.

When we moved onto the time series graph we ran into some issues in the data to which we discussed in class, with our findings being there is somehow a major error in the data showing from 2012-2018 there being over 900,000% veterans, which is obviously not possible and clearly an error in the data set, however for the years after 2018 it shows what we think are correct values at least and it shows out code is correct and works.

Finally for the scatter plot, we decided to compare veteran status with poverty levels on Oahu. The data for this seemed to work fine with no issues. And from what I can tell there is very little to no overall correlation in regards to poverty levels vs veteran status on Oahu.
